...
func TestStreamingProgressiveOutput(t *testing.T) {
	...
	t.Run("no flush mode", func(t *testing.T) {
		...
		err := Flow(ctx, r, w, Buffered, renderer)
		if err != nil {
			t.Fatalf("Flow error: %v", err)
		}
...
func TestStreamingBufferBoundaries(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Test with different window sizes - output should be identical
			ctx := context.Background()
			renderer := func(data []byte) ([]byte, error) {
				return data, nil
			}
			...
			err := Flow(ctx, r1, &buf1, tt.window, renderer)
			if err != nil {
			...
			err = Flow(ctx, r2, &buf2, Buffered, renderer)
			if err != nil {
			...
			if buf1.String() != buf2.String() {
				t.Errorf("Different outputs for different window sizes:\nSmall: %q\nNo window: %q",
...
func TestStreamingMemoryBounds(t *testing.T) {
	...
	t.Run("huge single line", func(t *testing.T) {
		...
		renderer := func(data []byte) ([]byte, error) {
			// Check that chunks are bounded
			if len(data) > Windowed {
				t.Errorf("Chunk size %d exceeds max window %d", len(data), Windowed)
			}
			return data, nil
		}

		err := Flow(ctx, r, &buf, 4096, renderer)
...
func TestStreamingLatency(t *testing.T) {
	t.Run("first byte latency", func(t *testing.T) {
		...
		for {
			...
			default:
				if len(w.writes) > 0 {
					elapsed := w.writes[0].timestamp.Sub(start)
