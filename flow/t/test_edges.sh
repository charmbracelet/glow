...
EOF
  ...
  timeout 1s cat /tmp/fence_bug.md | ./glow -w0 --flow=50 -w0 - > /tmp/fence_out.txt 2>&1 || true

  # Output should exist and contain content
  test -s /tmp/fence_out.txt && grep -q "hello" /tmp/fence_out.txt
...
EOF
  ...
  cat /tmp/fence_levels.md | ./glow -w0 --flow=30 - > /tmp/level1.txt 2>&1
  cat /tmp/fence_levels.md | ./glow -w0 --flow=-1 - > /tmp/level2.txt 2>&1

  diff -u /tmp/level1.txt /tmp/level2.txt
...
EOF
  ...
  cat /tmp/aligned.md | ./glow -w0 --flow=20 - > /tmp/align1.txt 2>&1
  cat /tmp/aligned.md | ./glow -w0 --flow=-1 - > /tmp/align2.txt 2>&1

  # Both should produce valid output
  test -s /tmp/align1.txt && test -s /tmp/align2.txt
...
EOF
  ...
  cat /tmp/pipes.md | ./glow -w0 --flow=30 - > /tmp/pipes.txt 2>&1

  # Should produce output without errors
  test -s /tmp/pipes.txt
...
EOF
  ...
  cat /tmp/code_table.md | ./glow -w0 --flow=15 - > /tmp/ct1.txt 2>&1
  cat /tmp/code_table.md | ./glow -w0 --flow=-1 - > /tmp/ct2.txt 2>&1

  # Both should treat table as code, producing identical output
  diff -u /tmp/ct1.txt /tmp/ct2.txt
...
EOF
  ...
  cat /tmp/split_table.md | ./glow -w0 --flow=25 - > /tmp/split1.txt 2>&1
  cat /tmp/split_table.md | ./glow -w0 --flow=-1 - > /tmp/split2.txt 2>&1

  # Should handle gracefully even if output differs
  test -s /tmp/split1.txt && test -s /tmp/split2.txt
...
EOF
  ...
  cat /tmp/meta.md | ./glow -w0 --flow=40 - > /tmp/meta1.txt 2>&1
  cat /tmp/meta.md | ./glow -w0 --flow=-1 - > /tmp/meta2.txt 2>&1

  # Should properly close all fences
  diff -u /tmp/meta1.txt /tmp/meta2.txt
...
EOF
  ...
  cat /tmp/deep.md | ./glow -w0 --flow=20 - > /tmp/deep.txt 2>&1
