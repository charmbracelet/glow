...
it should produce same output whether via files or pipes; (
  ...
  ((i==X || X<0)) && set -x; set -e
  timeout 0.5s ./glow -w0 --flow=-1 README.md > /tmp/file_out.txt 2>&1
  cat README.md | timeout 0.5s ./glow -w0 --flow=-1 - > /tmp/pipe_out.txt 2>&1
  diff -u /tmp/file_out.txt /tmp/pipe_out.txt
  rm -f /tmp/file_out.txt /tmp/pipe_out.txt
...
it should handle 1MB single line without newlines; (
  ...
  timeout 5s cat /tmp/huge_line.md | ./glow -w0 --flow=4096 - > /tmp/huge_out.txt 2>&1
  # Verify it processed something (even if truncated)
  test -s /tmp/huge_out.txt
  rm -f /tmp/huge_line.md /tmp/huge_out.txt
...
it should maintain consistency across buffer sizes; (
  ...
  # All outputs should match unlimited buffer version
  diff -u /tmp/buf_unlimited.out /tmp/buf_1k.out
  diff -u /tmp/buf_unlimited.out /tmp/buf_10.out
